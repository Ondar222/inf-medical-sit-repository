...
const LastPosts = (params) => {
  return (
    <div className="last-news: mt-3">
      <Link href="/news">
        <a className="font-semibold text-xl text-blue-500 tracking-wider pl-2">
          Все новости
        </a>
      </Link>
      <div className="last-news__container">
        {params?.news?.map((item) => {
          return (
            <div
              key={item.id}
              className="last-news__card first:relative first:flex-[0_0_auto] w-1/3 first:w-2/3 h-full md:first:h-[400px] odd:px-2 even: mt-5 "
            >
